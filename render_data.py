...
def Data():
    ...
    BASE_H = int(args[1])
    SS_RES = int(args[3])
    ALIAS_DIR = f"//renders/{BASE_H}/alias/"
    SSAA_DIR  = f"//renders/{BASE_H}/ssaa{SS_RES}x/"
    N_FRAMES = int(args[2])
    SKIP_EXISTING = False
    ...
    for i in range(N_FRAMES):
        ...
        # SS
        ssaa_path = bpy.path.abspath(SSAA_DIR + f"{comp_prefix}_frame{i:04d}_ssaa{SS_RES}x.png")
        if not SKIP_EXISTING or not os.path.exists(ssaa_path):
            scene.render.resolution_percentage = int(math.sqrt(SS_RES)) * 100
